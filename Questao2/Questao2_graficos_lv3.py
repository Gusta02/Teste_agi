...
# Lista única de razões sociais
razoes_sociais = data_filtered['razaosocial'].unique()

# Cria o aplicativo Dash
app = dash.Dash(__name__)

# Layout do aplicativo com o filtro de razões sociais
app.layout = html.Div([
        html.Div([
            dcc.Dropdown(
                id='razao-social-dropdown',
                options=[{'label': razao, 'value': razao} for razao in razoes_sociais],
...
# Função para atualizar os gráficos com base na página selecionada, no código do centro de custo e na razão social
@app.callback(
    ...
    [Input('page-slider', 'value'),
     ...
     Input('razao-social-dropdown', 'value')],
    [State('center-input', 'value')]
)
def update_graph(page_num, submit_n_clicks, button_n_clicks, selected_razao_social, center_code):
    # Filtra por razão social, se uma for selecionada
    if selected_razao_social:
        data_filtered_razao_social = data_filtered[data_filtered['razaosocial'] == selected_razao_social]
    else:
        data_filtered_razao_social = data_filtered

    # Gráfico de Barras: Contagem de Colaboradores por Centro de Custo
    grouped_data = data_filtered_razao_social.groupby('codigocentrocusto')['nome'].count().reset_index()
    ...
    if center_code is not None:
        center_data = data_filtered_razao_social[data_filtered_razao_social['codigocentrocusto'] == center_code]
